#!/usr/bin/env python3
"""Assemble a personalized skill-creator from the boilerplate's answers.
...
their tokens, and writes a ready-to-use folder named by `tool_name`
(`my-skill-creator` unless the user chose otherwise).

Deliberate limits:

- Does not overwrite. If that folder already exists, it aborts. The
  boilerplate is meant to run once; silently replacing a configured skill
...
- Does not write the lock file. That is the boilerplate's job, after it has
...
def generate(config_path, destination):
    ...
    if os.path.exists(root):
        sys.exit(f"'{root}' already exists -- aborting, nothing overwritten.\n"
                 "The boilerplate is meant to run once. To start over, delete "
